...
glm::vec3 compute_phong_lighting_flat(const glm::vec3& pos, const glm::vec3& normal) {
...
void render() {
	...
	create_scene(); // fill gVertexBuffer, gIndexBuffer
	...
	for (int i = 0; i < gNumTriangles; ++i) {
		...
		glm::vec4 wv0 = model * glm::vec4(gVertexBuffer[3 * k0 + 0], gVertexBuffer[3 * k0 + 1], gVertexBuffer[3 * k0 + 2], 1.0f);
		glm::vec4 wv1 = model * glm::vec4(gVertexBuffer[3 * k1 + 0], gVertexBuffer[3 * k1 + 1], gVertexBuffer[3 * k1 + 2], 1.0f);
		glm::vec4 wv2 = model * glm::vec4(gVertexBuffer[3 * k2 + 0], gVertexBuffer[3 * k2 + 1], gVertexBuffer[3 * k2 + 2], 1.0f);
		glm::vec3 centroid = glm::vec3((wv0 + wv1 + wv2) / 3.0f);
		glm::vec3 normal = glm::normalize(glm::cross(glm::vec3(wv1 - wv0), glm::vec3(wv2 - wv0)));

		glm::vec3 color = compute_phong_lighting_flat(centroid, normal);
