...
void render() {
	...
	std::vector<glm::vec3> worldPositions(gNumVertices);
	std::vector<glm::vec3> worldNormals(gNumVertices);
	...
	for (int i = 0; i < gNumVertices; ++i) {
		...
		worldPositions[i] = glm::vec3(model * v);
		...
		worldNormals[i] = glm::normalize(normalMatrix * normal_model);
	}
	...
	for (int i = 0; i < gNumTriangles; ++i) {
		...
		glm::vec3 p0 = worldPositions[k0];
		glm::vec3 p1 = worldPositions[k1];
		glm::vec3 p2 = worldPositions[k2];

		glm::vec3 n0 = worldNormals[k0];
		glm::vec3 n1 = worldNormals[k1];
		glm::vec3 n2 = worldNormals[k2];
			...
			for (int x = minX; x <= maxX; ++x) {
				...
				if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
					...
					if (z < DepthBuffer[idx]) {
						...
						glm::vec3 interpPos = w0 * p0 + w1 * p1 + w2 * p2;
						glm::vec3 interpNormal = glm::normalize(w0 * n0 + w1 * n1 + w2 * n2);

						glm::vec3 color = compute_phong_lighting_raw(interpPos, interpNormal);
